...
with tab3:
    ...
    sort_options = [c for c in ["popularity_songs", "danceability", "energy", "tempo", "valence"] if c in df_filtered.columns]
    sort_col = st.selectbox("Sort by", sort_options)
    show_cols = ["name_song", "name_artists", "cluster_label", "popularity_songs"] + FEATURES
    show_cols = [c for c in show_cols if c in df_filtered.columns]  # tolerate missing optional columns
    st.dataframe(
        df_filtered.sort_values(sort_col, ascending=False)[show_cols].head(200),
